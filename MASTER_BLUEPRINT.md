...
| 14 | 2026-06-10 | User: backtest every pool bot head-to-head | `baba_bot/battle.py` + `bot_battle.py`: 11 strategies faithfully reimplemented from audited source (FXBot×4, ForexSmartBot×3 technicals, geraked×2 pure entries, KB#4 MA-cross, KB#8/FXBot logistic-on-lags) on identical data/costs/rules; author-default params, no tuning; always-in vs SL/TP styles simulated per each bot's real behavior | geraked tested WITHOUT grid (martingale banned §7 — their READMEs admit pure = unprofitable, we verify); ForexSmartBot ML excluded (leaky targets = nothing real to test); Trading_Pal & EarnForex excluded (no strategies) |
| 15 | 2026-06-10 | **Bot battle results** (user-run, ~4y M15 × 4 pairs, 44 tests) | Only 2 non-degenerate winners, each regime-specific: (a) FXBot Bollinger-flat@mid positive ONLY on ranging majors (GBPUSD +0.41 pips/trade PF 1.05 n=3752; EURUSD +0.35 PF 1.05 n=3768), loses on trending assets; (b) KB#4 slow MA-cross 32/64 positive ONLY on trending assets (GOLD +15.8 pips/trade PF 1.22 n=1740; USDJPY +1.47 PF 1.07), loses on ranging majors — and GOLD total ≈ buy-and-hold of the 2022-26 secular bull (mostly trend beta, not alpha). geraked pure entries confirmed unprofitable everywhere (their own admission verified). Naked Donchian-20 lost on all four pairs — unfiltered breakouts die to spread/whipsaw; our filtered HIGH_VOL regime remains the breakout lead. Logistic-on-lags degenerated to constant-direction (= buy & hold) — KB#8's negative ML conclusion re-confirmed. RSI-reversion USDJPY +13.6/PF 2.02 but n=13 — anecdote, not evidence | **Empirically reproduces the blueprint's core regime thesis**: mean reversion wins on ranging pairs, slow trend-following wins on trending assets, nothing wins everywhere → strategy-per-regime routing is the architecture; next build = regime-matched triggers (slow-trend for GOLD/USDJPY TRENDING, BB-mid-revert for EURUSD/GBPUSD RANGING) |
| 16 | 2026-06-10 | **16-year battle with honest spreads + year-by-year** (the tournament verdict) | GOLD: nothing survives 17y — BB-fade profits were ALL 2009-2015 followed by 11 straight losing years (dead microstructure, not a cycle; year-split feature prevented transplanting it); trend logic ≈ beta. USDJPY: confirmed trend pair — 3 independent trend strategies positive over 16y (MA32/64 +1.13/trade, 10/17 yrs) converging with our own +0.14R walk-forward. EURUSD: confirmed mean-reversion pair — BB fade green in ALL 4 runs (16y: +0.71/trade, PF 1.05, n=3903), BBRSI variant +5.07 (10/17 yrs). GBPUSD: dropped (nothing in 16y). RSI-reversion: killed by first big sample (PF 0.07, n=100). **Triggers v2 shipped:** TRENDING = 32/64 SMA cross (replaces dead 9/21 EMA); RANGING = symmetric band re-entry fade, stop 0.9×half-band, target mid-band, fade-specific confluence scoring, 4H-bias gate suspended in ranges; HIGH_VOL unchanged; defaults → H1 decision TF, symbols GOLD/USDJPY/EURUSD | Bias-gate suspension in RANGING amends §6 (evidence: symmetric fade outperformed biased fade everywhere); battle spread now median-historical (off-hours snapshot poisoned one run — caught and rerun); pending: walk-forward re-validation of v2 triggers |
